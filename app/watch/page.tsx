                ...
                {activeTracks.length > 1 && (
                    <div className="flex gap-2 mb-3 overflow-x-auto pb-1">
                        {activeTracks.map((track, i) => {
                            ...
                            return (
                                <button key={i} onClick={() => setActiveTrack(i)} className={`angle-chip flex-shrink-0 ${i === activeTrack ? 'active' : 'inactive'}`}>{label}</button>
                            );
                        })}
                    </div>
                )}
                <div className="flex items-center justify-between">
                    <div className="text-xs text-white/30">{activeTracks.length} camera{activeTracks.length !== 1 && 's'} connected</div>
                    <div className="flex items-center gap-2">
                        {clipStart !== null ? (
                            <>
                                <span className="text-xs text-amber-400 font-bold animate-pulse">● Clipping: {fmt(elapsed - clipStart)}</span>
...
function WatchInner() {
    ...
    useEffect(() => {
        ...
        async function init() {
            ...
            setGame(g);
            const t = await fetchToken({ roomName: g.roomName, participantName: `viewer-${Date.now()}`, role: 'viewer' });
            setToken(t);
            setLoading(false);
        }
        init();
    ...
    return (
        <div className="relative">
            ...
            {clipToast && (
                <div className="fixed bottom-20 left-1/2 -translate-x-1/2 px-4 py-2.5 bg-emerald-500 text-black text-sm font-black rounded-full shadow-lg animate-fade-in">{clipToast}</div>
            )}
        </div>
    );
}
